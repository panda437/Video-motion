#!/usr/bin/env node
/**
 * Captures the ARM video page at 1280x720, then merges with the audio.
 * Output: output/arm_final.mp4
 *
 * Usage: node scripts/capture-arm.js
...
const AUDIO_FILE   = path.join(process.env.HOME, "Downloads/ARM audio.m4a");
const BGM_FILE     = path.join(process.env.HOME, "Downloads/arm_bgm.mp3");
const OUTPUT_DIR   = path.join(__dirname, "../output");
const MP4_VIDEO    = path.join(OUTPUT_DIR, "arm_video.mp4");
const FINAL_OUTPUT = path.join(OUTPUT_DIR, "arm_final.mp4");
const PAGE_URL     = "http://localhost:3010/videos/arm?cinema=1";

// Sum of scene durations is ~579s
const TOTAL_MS = 579_080;
...
(async () => {
  ...
  if (fs.existsSync(BGM_FILE)) {
    ffmpeg([
      ...
      "-i", BGM_FILE,
      "-filter_complex",
      "[2:a]volume=0.15[bgm_base]; [bgm_base][1:a]sidechaincompress=threshold=0.1:ratio=20:attack=200:release=1000[bgm_ducked]; [1:a][bgm_ducked]amix=inputs=2:duration=first[a]",
